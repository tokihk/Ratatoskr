...
namespace Ratatoskr.Configs.UserConfigs
{
    ...
    [Serializable]
    internal sealed class NotifyMailConfig
    {
        ...
        public NotifyMailConfig(Guid id, string smtp_host, uint smtp_port, SmtpConnectModeType mode, string smtp_username, string smtp_password)
        {
            ID = id;
            SmtpHost = smtp_host;
            SmtpPort = smtp_port;
        ...
        public NotifyMailConfig() { }
    }

    [Serializable]
    internal sealed class NotifyMailListConfig : IConfigDataReadOnly<List<NotifyMailConfig>>, IConfigReader, IConfigWriter
    {
        private const    string XML_NODE_DATA   = "data";
        private readonly string CRYPTO_PASSCODE = "E1DD0BA3-5D35-4246-A467-1E33C65960E9";


        public List<NotifyMailConfig> Value { get; } = new List<NotifyMailConfig>();


        public NotifyMailListConfig()
        {
        }
        ...
        private void LoadConfigPart_Data(XmlElement xml_node)
        {
            ...
            var newobj = new NotifyMailConfig();

            /* === パラメータ読み込み === */
            /* id */
            newobj.ID = new Guid(XmlUtil.GetAttribute(xml_node, "id", Guid.NewGuid().ToString()));

            /* snmp-host */
            newobj.SmtpHost = XmlUtil.GetAttribute(xml_node, "smtp-host", "");

            /* snmp-port */
            newobj.SmtpPort = uint.Parse(XmlUtil.GetAttribute(xml_node, "smtp-port", ""));

            /* snmp-username */
            newobj.SmtpUsername = XmlUtil.GetAttribute(xml_node, "smtp-username", "");

            /* snmp-password */
            newobj.SmtpPassword = XmlUtil.GetAttribute(xml_node, "smtp-password", "");
            if (newobj.SmtpPassword.Length > 0) {
                newobj.SmtpPassword = CryptoUtil.DecryptText(newobj.SmtpPassword, CRYPTO_PASSCODE);
            }

            /* minimum-interval */
            newobj.MinimumInterval = uint.Parse(XmlUtil.GetAttribute(xml_node, "minimum-interval", "180"));

            /* notify-timming-mask */
            newobj.NotifyTimmingMask = XmlUtil.GetAttribute(xml_node, "notify-timming-mask", "");
