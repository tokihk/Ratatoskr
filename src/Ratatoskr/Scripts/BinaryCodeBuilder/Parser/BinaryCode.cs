...
{
    internal sealed class BinaryCode
    {
        private const int DATA_BUFFER_MARGIN_SIZE = 128;


        public static BinaryCode operator+ (BinaryCode left, BinaryCode right)
        {
            /* Leftに十分な空きがないときは拡張 */
        ...
        public BinaryCode(byte[] data_buffer)
        {
            data_buffer_ = data_buffer;
            data_size_ = data_buffer.Length;
        }

        public BinaryCode(byte[] data_buffer, int data_size)
        {
            data_buffer_ = data_buffer;
            data_size_ = data_size;
        }

        public BinaryCode(ulong value)
        {
            data_buffer_ = BitConverterEx.GetBytesBigEndian(value);
            data_size_ = data_buffer_.Length;
        }

        public BinaryCode(string hex_string)
        {
            data_buffer_ = HexTextEncoder.ToBinary(hex_string);
            data_size_ = data_buffer_.Length;
        }

        public BinaryCode(Encoding encoder, string text)
        {
