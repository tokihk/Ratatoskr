...
namespace Ratatoskr.Devices.SerialPort
{
    internal unsafe sealed class DeviceInstanceImpl : DeviceInstance
    {
...
#else
			wait_event_task_ar_ = (new WaitEventTaskDelegate(WaitEventTask)).BeginInvoke(null, null);
#endif
        }
		...
		protected override EventResult OnDisconnectBusy()
		{
			...
			/* タスク終了待ち */
			return ((wait_event_task_ar_.IsCompleted) ? (EventResult.Success) : (EventResult.Busy));
		}
...
//                    DeviceCommTask();
        ...
        private void SendPoll(ref bool busy)
        {
            ...
            SendComplete(send_data_, send_size);
        ...
        private void SendComplete(byte[] send_data, uint send_size)
        {
            ...
            if (send_data.Length > send_size) {
                send_data = ClassUtil.CloneCopy(send_data_, (int)send_size);
		...
		private delegate void DeviceSendTaskDelegate();
		private void DeviceSendTask()
		{
            var send_data = send_data_;
