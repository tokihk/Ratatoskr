...
{
    [Serializable]
    public sealed class EnumListConfig<T> : ConfigObject, IConfigDataReadOnly<List<T>>, IConfigReader, IConfigWriter
        where T : struct
    {
        ...
        private void LoadConfigPart_Data(XmlElement xml_node)
        {
            ...
            /* === 設定リストへ追加 === */
            if (Enum.TryParse<T>(GetAttribute(xml_node, "value", ""), out T value)) {
                Value.Add(value);
            }
        }
